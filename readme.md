...
- Error: 400(GraphQL 오류), 404(CSRF 없음)
...
- Response: `{ data: <graphql result> }` → 목록은 `data.data.discussList.list`
- Error: 400(필수값 누락/GraphQL 오류), 404(CSRF 없음)
...
- Error: 400(파라미터 누락/GraphQL), 404(User not found), 500(Internal)

## 참고 파일

- 헤더 유틸: [lib/headers.js](lib/headers.js)
- GraphQL 쿼리: [lib/graphql.js](lib/graphql.js)
- 세션/쿠키 클라이언트: [lib/httpSession.js](lib/httpSession.js)
